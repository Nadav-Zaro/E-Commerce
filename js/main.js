var furnitures = [
    {
        id: 1,
        furName: "Alfred",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id1.jpg",
            "../images/id11.jpg"
        ]
    },
    {
        id: 2,
        furName: "Aston",
        price: 1500,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id2.jpg",
            "../images/id21.jpg"
        ]
    },
    {
        id: 3,
        furName: "Bellevue",
        price: 2600,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id31.jpg",
            "../images/id3.jpg"
        ]
    },
    {
        id: 4,
        furName: "Border",
        price: 1500,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id4.jpg",
            "../images/id41.jpg"
        ]
    },
    {
        id: 5,
        furName: "Brandy",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id5.jpg",
            "../images/id51.jpg"
        ]
    },
    {
        id: 6,
        furName: "Dalinda",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id6.jpg",
            "../images/id61.jpg"
        ]
    },
    {
        id: 7,
        furName: "Batman",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id1.jpg",
            "../images/id11.jpg"
        ]
    },
    {
        id: 8,
        furName: "Superman",
        price: 1500,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id2.jpg",
            "../images/id21.jpg"
        ]
    },
    {
        id: 9,
        furName: "Wonder Woman",
        price: 2600,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id3.jpg",
            "../images/id31.jpg"
        ]
    },
    {
        id: 10,
        furName: "Black panther",
        price: 1500,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id4.jpg",
            "../images/id41.jpg"
        ]
    },
    {
        id: 11,
        furName: "Hulk",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id5.jpg",
            "../images/id51.jpg"
        ]
    },
    {
        id: 12,
        furName: "IronMan",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id6.jpg",
            "../images/id61.jpg"
        ]
    },
    {
        id: 13,
        furName: "Spider-Man",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id1.jpg",
            "../images/id11.jpg"
        ]
    },
    {
        id: 14,
        furName: "Thor",
        price: 1500,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id2.jpg",
            "../images/id21.jpg"
        ]
    },
    {
        id: 15,
        furName: "Locky",
        price: 2600,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id3.jpg",
            "../images/id31.jpg"
        ]
    },
    {
        id: 16,
        furName: "Dr.S",
        price: 1500,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id4.jpg",
            "../images/id41.jpg"
        ]
    },
    {
        id: 17,
        furName: "Captain Marvel",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id5.jpg",
            "../images/id51.jpg"
        ]
    },
    {
        id: 18,
        furName: "Groot",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id6.jpg",
            "../images/id61.jpg"
        ]
    },
    {
        id: 19,
        furName: "Dominik",
        price: 10000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id19.jpg",
            "../images/id191.jpg"
        ]
    }, {
        id: 20,
        furName: "Nano",
        price: 12000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id192.jpg",
            "../images/id193.jpg"
        ]
    },
    {
        id: 21,
        furName: "Leo",
        price: 11500,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id194.jpg",
            "../images/id195.jpg"
        ]
    },
    {
        id: 22,
        furName: "Maru",
        price: 15000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id196.jpg",
            "../images/id197.jpg"
        ]
    },
    {
        id: 23,
        furName: "Almon",
        price: 14600,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id198.jpg",
            "../images/id199.jpg"
        ]
    },
    {
        id: 24,
        furName: "Zaro",
        price: 20000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id1910.jpg",
            "../images/id1911.jpg"
        ]
    },
    {
        id: 25,
        furName: "Zauda",
        price: 10000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id19.jpg",
            "../images/id191.jpg"
        ]
    }, {
        id: 26,
        furName: "Arka",
        price: 12000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id192.jpg",
            "../images/id193.jpg"
        ]
    },
    {
        id: 27,
        furName: "Dassa",
        price: 11500,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id194.jpg",
            "../images/id195.jpg"
        ]
    },
    {
        id: 28,
        furName: "Mocha",
        price: 15000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id196.jpg",
            "../images/id197.jpg"
        ]
    },
    {
        id: 29,
        furName: "Kassa",
        price: 14600,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id198.jpg",
            "../images/id199.jpg"
        ]
    },
    {
        id: 30,
        furName: "Vanda",
        price: 20000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id1910.jpg",
            "../images/id1911.jpg"
        ]
    },
    {
        id: 31,
        furName: "Laker",
        price: 10000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id19.jpg",
            "../images/id191.jpg"
        ]
    },
    {
        id: 32,
        furName: "Hornet",
        price: 12000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id192.jpg",
            "../images/id193.jpg"
        ]
    },
    {
        id: 33,
        furName: "Clipper",
        price: 11500,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id194.jpg",
            "../images/id195.jpg"
        ]
    },
    {
        id: 34,
        furName: "Raptor",
        price: 15000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id196.jpg",
            "../images/id197.jpg"
        ]
    },
    {
        id: 35,
        furName: "Warrior",
        price: 14600,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id198.jpg",
            "../images/id199.jpg"
        ]
    },
    {
        id: 36,
        furName: "Nugget",
        price: 20000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id1910.jpg",
            "../images/id1911.jpg"
        ]
    },
    {
        id: 37,
        furName: "Thunder",
        price: 5000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id37.jpg",
            "../images/id371.jpg"
        ]
    },
    {
        id: 38,
        furName: "Grizzly",
        price: 4500,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id372.jpg",
            "../images/id373.jpg"
        ]
    },
    {
        id: 39,
        furName: "PARA",
        price: 6000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id374.jpg",
            "../images/id375.jpg"
        ]
    },
    {
        id: 40,
        furName: "Mina",
        price: 9600,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id376.jpg",
            "../images/id377.jpg"
        ]
    },
    {
        id: 41,
        furName: "Marco",
        price: 2400,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id378.jpg",
            "../images/id379.jpg"
        ]
    },
    {
        id: 42,
        furName: "Zennette",
        price: 7000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id3710.jpg",
            "../images/id3711.jpg"
        ]
    },
    {
        id: 43,
        furName: "Lemon",
        price: 5000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id37.jpg",
            "../images/id371.jpg"
        ]
    },
    {
        id: 44,
        furName: "Flex",
        price: 4500,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id372.jpg",
            "../images/id373.jpg"
        ]
    },
    {
        id: 45,
        furName: "Rocket",
        price: 6000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id374.jpg",
            "../images/id375.jpg"
        ]
    },
    {
        id: 46,
        furName: "Sun",
        price: 9600,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id376.jpg",
            "../images/id377.jpg"
        ]
    },
    {
        id: 47,
        furName: "Mavrik",
        price: 2400,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id378.jpg",
            "../images/id379.jpg"
        ]
    },
    {
        id: 48,
        furName: "Trail=Blazer",
        price: 7000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id3710.jpg",
            "../images/id3711.jpg"
        ]
    },
    {
        id: 49,
        furName: "Buck",
        price: 5000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id37.jpg",
            "../images/id371.jpg"
        ]
    },
    {
        id: 50,
        furName: "Heat",
        price: 4500,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id372.jpg",
            "../images/id373.jpg"
        ]
    },
    {
        id: 51,
        furName: "Wizard",
        price: 6000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id374.jpg",
            "../images/id375.jpg"
        ]
    },
    {
        id: 52,
        furName: "Net",
        price: 9600,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id376.jpg",
            "../images/id377.jpg"
        ]
    },
    {
        id: 53,
        furName: "Pacer",
        price: 2400,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id378.jpg",
            "../images/id379.jpg"
        ]
    },
    {
        id: 54,
        furName: "Wolve",
        price: 7000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id3710.jpg",
            "../images/id3711.jpg"
        ]
    },
    {
        id: 55,
        furName: "Celine",
        price: 1200,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id55.jpg",
            "../images/id551.jpg"
        ]
    },
    {
        id: 56,
        furName: "Dion",
        price: 1400,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id552.jpg",
            "../images/id553.jpg"
        ]
    },
    {
        id: 57,
        furName: "Riri",
        price: 70000,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id554.jpg",
            "../images/id555.jpg"
        ]
    },
    {
        id: 58,
        furName: "B",
        price: 12000,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id556.jpg",
            "../images/id557.jpg"
        ]
    },
    {
        id: 59,
        furName: "Brez",
        price: 8500,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id558.jpg",
            "../images/id559.jpg"
        ]
    },
    {
        id: 60,
        furName: "Eluse",
        price: 1200,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id5510.jpg",
            "../images/id5511.jpg"
        ]
    },
    {
        id: 61,
        furName: "Bacelona",
        price: 1200,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id55.jpg",
            "../images/id551.jpg"
        ]
    },
    {
        id: 62,
        furName: "Madrid",
        price: 1400,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id552.jpg",
            "../images/id553.jpg"
        ]
    },
    {
        id: 63,
        furName: "Munich",
        price: 70000,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id554.jpg",
            "../images/id555.jpg"
        ]
    },
    {
        id: 64,
        furName: "Paris",
        price: 12000,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id556.jpg",
            "../images/id557.jpg"
        ]
    },
    {
        id: 65,
        furName: "Roma",
        price: 8500,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id558.jpg",
            "../images/id559.jpg"
        ]
    },
    {
        id: 66,
        furName: "United",
        price: 1200,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id5510.jpg",
            "../images/id5511.jpg"
        ]
    },
    {
        id: 67,
        furName: "City",
        price: 1200,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id55.jpg",
            "../images/id551.jpg"
        ]
    },
    {
        id: 68,
        furName: "Juve",
        price: 1400,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id552.jpg",
            "../images/id553.jpg"
        ]
    },
    {
        id: 69,
        furName: "Inter",
        price: 70000,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id554.jpg",
            "../images/id555.jpg"
        ]
    },
    {
        id: 70,
        furName: "Dourtmond",
        price: 12000,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id556.jpg",
            "../images/id557.jpg"
        ]
    },
    {
        id: 71,
        furName: "Monacco",
        price: 8500,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id558.jpg",
            "../images/id559.jpg"
        ]
    },
    {
        id: 72,
        furName: "Lafayete",
        price: 1200,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id5510.jpg",
            "../images/id5511.jpg"
        ]
    }, {
        id: 73,
        furName: "Joker",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id5.jpg",
            "../images/id51.jpg"
        ]
    },
    {
        id: 74,
        furName: "Antman",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "../images/id6.jpg",
            "../images/id61.jpg"
        ]
    },
    {
        id: 75,
        furName: "Black-Mamba",
        price: 11500,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id194.jpg",
            "../images/id195.jpg"
        ]
    },
    {
        id: 76,
        furName: "LBJ",
        price: 15000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "../images/id196.jpg",
            "../images/id197.jpg"
        ]
    },
    {
        id: 77,
        furName: "Air j",
        price: 2400,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id378.jpg",
            "../images/id379.jpg"
        ]
    },
    {
        id: 78,
        furName: "Larka",
        price: 7000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "../images/id3710.jpg",
            "../images/id3711.jpg"
        ]
    },
    {
        id: 79,
        furName: "Lumeir",
        price: 8500,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id558.jpg",
            "../images/id559.jpg"
        ]
    },
    {
        id: 80,
        furName: "Alfon",
        price: 1200,
        description: "most serene",
        category: "carpets",
        pictures: [
            "../images/id5510.jpg",
            "../images/id5511.jpg"
        ]
    },

]

var customerCart1 = [
    {
        id: 1,
        furName: "Alfred",
        price: 2000,
        description: "most compfortable",
        category: "living-room",
        pictures: [
            "./images/id1.jpg",
            "./images/id11.jpg"
        ]
    },
    {
        id: 31,
        furName: "Dominik",
        price: 10000,
        description: "most astonishing",
        category: "kitchens",
        pictures: [
            "./images/id19.jpg",
            "./images/id191.jpg"
        ]
    },
    {
        id: 37,
        furName: "Lemon",
        price: 5000,
        description: "most spectacular",
        category: "dinette",
        pictures: [
            "./images/id37.jpg",
            "./images/id371.jpg"
        ]
    },
]

var cartPop = document.getElementById("cartPop");
var cartPopFurs = document.getElementById("cartPopFurs");
var cartCounter = document.getElementById("cartCounter");
var CartIcon = document.getElementById("CartIcon");

for (let i = 0; i < customerCart1.length; i++) {
    var randomPrice = Math.floor((Math.random() * 4000) + 6000);
    cartPopFurs.innerHTML += `
    <div class="cartPopItems">
    <div class="cartImgHolder">
    <img class="cartImg" src="${customerCart1[i].pictures[0]}">
    </div>
    <div class="cartfurInfo">
    <p>"${customerCart1[i].furName}"</p>
    <p>category: ${customerCart1[i].category}</p>
    <p>price: ${randomPrice}$</p>
    </div>
    <div class="cartRemove"><i class="fa fa-minus"></i></div>
    </div>`
}

var exitCart = document.getElementById("exitCart");
var cartRemove = document.getElementsByClassName("cartRemove");
var cartPopItems = document.getElementsByClassName("cartPopItems");
var furniture = document.getElementById("furniture");
var queryBtn = document.getElementById("queryBtn");
var queryLinks = document.getElementById("queryLinks");
var closeNav = document.getElementById("closeNav");
var surprise = document.getElementById("surprise");
var exitSale = document.getElementById("exitSale");

setTimeout(() => {
    surprise.style.visibility= "visible";
    exitSale.onclick=()=>{surprise.style.visibility= "hidden";}
}, 2000);

queryBtn.addEventListener("mouseenter",()=>{queryLinks.style.visibility = "visible"})
closeNav.addEventListener("click",()=>{queryLinks.style.visibility = "hidden"})

CartIcon.onclick = () => {
    cartPop.style.visibility = "visible";
    furniture.style.zIndex = "-1";
};

exitCart.onclick = () => {
    cartPop.style.visibility = "hidden";
    furniture.style.zIndex = "0";
};

for (let i = 0; i < cartPopItems.length; i++) {
    cartRemove[i].onclick = () => {
        cartPopItems[i].innerHTML = "";
        cartPopItems[i].style.borderTop = "0px";
        cartCounter.innerText--;
    }
}
